...
An `Objective-C`/`Swift` bindings for [LevelDB](https://code.google.com/p/leveldb/).

## Features

- Follows conventions of the `C++` API;
- Exposes all returned error codes;
- Exposes read and write `options`;
- Wraps `Iterator` interface in `NSEnumerator` subclass;
- Provides strong typed convenient wrappers for `Swift`;
- Provides `Generator`/`Sequence` protocol implementation for `Swift`;
